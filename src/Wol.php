...
class Wol extends EventEmitter
{
    ...
    public function __construct(DatagramSocket $socket)
    {
        $this->socket = $socket;
        $this->socket->pause();
    }
    ...
    public function send($mac) {
        $this->socket->send($this->composeMessage($mac));
    }
    ...
    public function composeMessage($mac) {
        $mac = $this->coerceMac($mac);

        return "\xFF\xFF\xFF\xFF\xFF\xFF" . str_repeat($this->formatMac($mac), 16);
    }
